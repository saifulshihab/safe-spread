...
test("Should return base if update is null", () => {
  const obj1 = { a: 1, b: 2 };
  const result = safeSpread(obj1, null as any);
  expect(result).toEqual(obj1);
});

test("Should return base if update is not an object", () => {
  const obj1 = { a: 1, b: 2 };
  const result = safeSpread(obj1, 42 as any);
  expect(result).toEqual(obj1);
});

test("Should return base if base is not an object", () => {
  const result = safeSpread(42 as any, { a: 1 });
...
test("Should not add new keys from update", () => {
  const obj1 = { a: 1 };
  const obj2 = { b: 2 };
  const result = safeSpread(obj1, obj2 as Partial<typeof obj1>);
...
test("Should deep merge only existing nested keys", () => {
  ...
  const result = safeSpread(obj1, obj2 as any);
  expect(result).toEqual({ a: { b: 10, c: 2 }, d: 4 });
});

test("Should handle base or update as undefined", () => {
  expect(safeSpread(undefined as any, { a: 1 })).toBe(undefined);
  expect(safeSpread({ a: 1 }, undefined as any)).toEqual({ a: 1 });
});
